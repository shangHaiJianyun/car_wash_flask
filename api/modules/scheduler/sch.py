...
def sort_jobs(jobs, order_interval, w_start, w_end):
    ...
    for addr in addrs:
        ...
        while len(job3) > 0:
            ...
            for t, row in job3.iterrows():
                ...
                job3.loc[t, 'grp'] = k
                if (plan_end + row.hrs_t + order_interval < row.end_time) & (
                        plan_end + max_wait_interval > row.start_time):
                    row.grp = k
    ...
    job_sorted.loc[:, 'wait_hr'] = np.where(job_sorted.grp.shift(1) == job_sorted.grp,
                                            (job_sorted.plan_start - job_sorted.plan_end.shift(1)) / np.timedelta64(1,
                                                                                                                    'h') - job_sorted.trans_hr,
                                            0)
                ...
                if len(jobs_a):
                    ...
                    if len(job_arranged) > 0:
                        ...
                        job_grp = job_grp.reset_index().sort_values(
                            ['hrs', 'idle_rate', 'duration', 'plan_start', 'addr', 'grp', ],
                            ascending=[0, 1, 0, 0, 1, 1])
